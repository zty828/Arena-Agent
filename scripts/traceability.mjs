...
const requirements=[
 ...
 ['B12','S01,S07','原理复现','packages/skills; apps/daemon/tools; apps/desktop','agentskills.io SKILL.md','T27','implemented','partial','规范实现：发现/校验/渐进披露；工作区外只读挂载；窗口内安装/删除（先校验后写、链接拒绝、不覆盖、原子落位）；容忍规范外字段','无 URL/zip 安装；无版本与依赖解析；未做跨工具目录的自动发现（需配置）'],
...
const tests=[
 ...
 ['T27','Skills/scripts cannot bypass approval','implemented','partial','scripts/probe-skills.mjs','skill 只读挂载、无专用执行入口；allowed-tools 只作信息展示，鉴权模块不认识该字段（有结构断言 + 变异测试覆盖）','若将来新增 run_skill_script，这条必须重新验证；未做执行沙箱本身'],
